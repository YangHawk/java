...
//JNDI(Java Naming Directory Interface): WAS 프로그램에 의해 관리되는 객체를 미리 생성하여 저장하고 웹프로그램에서 객체가 필요한 경우 WAS 프로그램에 등록된 객체의 이름을 이용하여 객체를 제공받아 사용하기 위한 기능
// → WAS 프로그램에 의해 관리되는 객체에 대한 정보를 src/main/webapp/META-INF/context.xml 파일에 작성

//WAS 프로그램에 등록된 DataSource 객체를 제공받아 DataSource 객체에 저장된 다수의 Connection 객체 중 하나를 제공받아 Connection 객체의 정보를 클라이언트에게 전달하여 응답하는 서블릿
...
public class JndiServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			DataSource dataSource = (DataSource) new InitialContext().lookup("java:comp/env/jdbc/oracle"); // java:comp/env/ 폴더 안에 자바 객체가 저장되어있다!
