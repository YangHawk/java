...
//→ 클라이언트에서 서버에 접속하여 웹 프로그램을 요청할 시 서버 정보로 저장된 모든 쿠키를 리퀘스트 메세지에 담아서 제공
...
public class CookieReadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// HttpServletRequest.getCookie(): 클라이언트에게 전달받은 모든 쿠키클 얻어와 Cookie 객체 배열로 반환하는 메소드
		...
		out.println("</head>");
		out.println("<body>");
		out.println("<h1>니가 만든 쿠키~ 읽기</h1>");
		out.println("<hr>");
		if (cookies == null) { // 클라이언트로부터 전달받은 쿠키가 없는 경우!
			out.println("<p>니가 만든 쿠키~ 없어!</p>");
		} else {
			String id = "";
			String count = ""; // 쿠기값을 저장하는 변수 선언!
		...
		out.println("<hr>");
		out.println("<p><a href='create.itwill'>니가 만든 쿠키~ 생성</a></p>");
		out.println("<p><a href='remove.itwill'>니가 만든 쿠키~ 삭제</a></p>");
		out.println("</body>");
		out.println("</html>");
