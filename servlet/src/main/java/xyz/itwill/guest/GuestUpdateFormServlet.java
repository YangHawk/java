...
//글번호를 전달받아 GUEST 테이블에 저장된 방명록 게시글을 검색하여 입력 태그의 초기값으로 설정하고 사용자로부터 방명록 게시글(변경값)을 입력받기 위한 서블릿
//→ [글변경] 태그를 클릭한 경우 방명록 게시글 변경 페이지(/guest/modify.itwill)로 이동 - 입력값 전달
//→ [글목록] 태그를 클릭한 경우 방명록 게시글 목록 페에지(/guest/list.itwill)로 이동
...
public class GuestUpdateFormServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// QueryString: /guest/modifyForm.itwill?num=123 식으로 전달
		out.println("<table>");
		out.println("<tr>");
