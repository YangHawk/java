...
//DAO(Data Access Object) 클래스: 테이블에 행 삽입, 삭제, 변경, 검색하는 SQL 명령을 전달하여 실행하고 실행 결과를 Java 객체(값)로 매핑하여 반환하는 기능을 제공하는 클래스
...
//STUDENT 테이블에 학생 정보를 삽입, 삭제, 변경, 검색하는 기능을 제공하는 클래스
	...
	public List<StudentDTO> selectStudentList() {
		...
		try {
			...
			while (rs.next()) {
				...
				// → ResultSet 커서 위치의 행에 대한 컬럼값은 DTO 객체의 필드에 저장
