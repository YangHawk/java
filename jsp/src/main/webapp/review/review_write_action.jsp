...
//전달 파일을 저장할 서버 디렉토리(웹 자원)의 파일 시스템 경로를 반환받아 저장
//String saveDirectory=application.getRealPath("/review_images"); //서블렛이 아닌 경우
String saveDirectory = request.getServletContext().getRealPath("/review_images");
System.out.println("saveDirectory = " + saveDirectory); //saveDirectory = C:\Temp\apache-tomcat-9.0.75\webapps\jsp\review_images

//MultipartRequest 객체 생성 - 모든 전달 파일을 서버 디렉토리에 업로드 처리하여 저장
...
//▶ 새글인 경우에는 게시글의 그룹번호=글그룹(REF 컬럼값)의 값으로 저장
...
if (ref == 0) { //새글인 경우
  //REVIEW 테이블의 REF 컬럼값에는 시퀸스의 다음값(num 변수값)을 저장하고 RESTEP 컬럼과 RELEVEL 컬럼에는 restep 변수값(0)과 relevel 변수값(0)을 저장
  ref = num;
} else { //답글인 경우
  //REVIEW 테이블에 저장된 기존 게시글에서 REF 컬럼값이 ref 변수값(부모글)과 같은 게시글 중 RESTEP 컬럼값이 restep 변수값(부모글)보다 큰 모든 게시글의 RESTEP 컬럼값을 1 증가되도록 변경 처리
...
//DTO 객체를 생성하고 전달값 또는 변수값으로 필드값 변경
