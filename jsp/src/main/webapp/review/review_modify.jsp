...
<form action="<%=request.getContextPath()%>/review/review_modify_action.jsp" method="post" enctype="multipart/form-data" id="reviewForm">
	<input type="hidden" name="num" value="<%=num%>"> <input type="hidden" name="pageNum" value="<%=pageNum%>"> <input type="hidden" name="search" value="<%=search%>"> <input type="hidden" name="keyword" value="<%=keyword%>">
	<table>
		<tr>
			<th>제목</th>
			<td><input type="text" name="subject" id="subject" size="40" value="<%=review.getSubject()%>"> <input type="checkbox" name="secret" value="2" <%if (review.getStatus() == 2) {%> checked="checked" <%}%>>비밀글</td>
		</tr>
		<tr>
			<th>내용</th>
			<td><textarea rows="7" cols="60" name="content" id="rContent"><%=review.getContent()%></textarea></td>
		</tr>
		<tr>
			<th>리뷰이미지</th>
