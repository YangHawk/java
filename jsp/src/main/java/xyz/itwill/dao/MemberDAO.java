...
public class MemberDAO extends JdbcDAO {
  ...
  // 아이디를 전달받아 MEMBER 테이블에 저장된 회원 정보의 마지막 로그인 날짜를 변경하고 변경한 행의 갯수를 반환하는 메소드
  ...
  // 아이디를 전달받아 MEMBER 테이블에 저장된 회원 정보의 변경하고 변경행갯수를 반환하는 메소드
  public int updateMember(MemberDTO member) {
    ...
    try {
      ...
      String sql =
          "update member set passwd=?, name=?, email=?, mobile=?, zipcode=?, address1=?, address2=? where id=?";
      pstmt = con.prepareStatement(sql);
