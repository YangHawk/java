...
public interface MyHewonMapper {
  ...
  // HashMap 클래스(Map 인터페이스)를 매개 변수로 설정할 경우 제네릭은 맵키를 [String] 클래스로 설정하고 맵값은 [Object] 클래스로 설정하여 사용
  ...
  // @Param: 추상 메소드의 매개 변수에 저장된 값을 XML 기반 매퍼 파일의 엘리먼트에게 전달하여 SQL 명령에서 사용할 수 있도록 제공하는 어노테이션
  // value 속성: 매개 변수에 저장된 값을 XML 기반 매퍼 파일의 엘리먼트에 등록된 SQL 명령에서 사용할 수 있는 식별자를 속성값으로 설정
  // ▶ 다름 속성이 없는 경우 속성값만 설정 가능
  ...
  List<MyHewon> selectSearchHewonList2(Map<String, Object> map);
