...
public class AccountController {
	...
	public String register(@ModelAttribute Account account, @RequestParam String accountRole)
			throws ExistsUserinfoException {
	 ...
	 * HttpSession session) { try { Account authAccount =
	 * accountService.loginAuth(account);
	 * 
	 * if(authAccount != null) { session.setAttribute("loginAccount", authAccount);
	 * // 세션에 사용자 정보 저장
	 * 
	 * if(autoLogin) { //자동 로그인 처리 } return new ResponseEntity<>("로그인 성공 메시지",
	 * HttpStatus.OK); } else { return new ResponseEntity<>("아이디 또는 비밀번호를 확인해주세요.",
	 * HttpStatus.UNAUTHORIZED); } } catch (LoginAuthFailException e) { return new
	 * ResponseEntity<>("아이디 또는 비밀번호를 확인해주세요.", HttpStatus.UNAUTHORIZED); } }
	 */
	 ...
	 * RequestMethod.POST}) public String logout(HttpServletRequest request){
	...
	public String findId(HttpServletRequest request, Model model,
			...
			@RequestParam(required = true, value = "email") String email, Account account) {
