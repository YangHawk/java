...
// ▶ 프로그램을 변경하지 않고 Properties 파일의 내용을 변경하여 프로그램 실행 결과를 변경 가능
// ▶ 프로그램의 유지보수 효율성 증가
...
public class PropertiesApp {
	public PropertiesApp() throws IOException {
		...
		// ClassLoader.getResourceAsStream(String name): 리소스 파일에 대한 입력스트림을 생성하는 메소드
		...
		// Properties.load(InputStream in): 입력 스트림으로 Properties 파일을 제공받아 파일에 저장된
		// 모든 이름과 값으로 Properties 객체에 엔트리르 추가하는 메소드
		properties.load(in);

		// Properteis.get(String Key): Properties 객체에 저장된 엔트리에서 맵키(MapKey)를 전달받아
		// 맵키와 매칭된 맵값(MapValue)를 반환하는 메소드
...
// 2.
// InputStream in3 = Class.forName("src/xyz/itwill/dbcp/PropertiesApp.java").getClassLoader().getResourceAsStream("src/xyz/itwill/dbcp/user.properties");
