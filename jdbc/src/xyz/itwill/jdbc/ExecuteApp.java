...
public class ExecuteApp {
	public static void main(String[] args) throws SQLException {
		 ...
		 * 
		 * 
		 * while (rs.next()) { System.out.println("학번 = " + rs.getInt("no") + ", 이름 = "
		 * + rs.getString("name")); }
		 * 
		 * 
		 * System.out.println("===========================================================");
		...
		int choice = 1; //int choice = 2; = false
		String sql = "";
		if (choice == 1) {
		...
		if(result) {//SELECT 명령이 전달되어 실행된 경우
			//Statement.getResultSet(): Statement 객체로 전달되어 실행된 SELECT
			//명령의 처리 결과를 ResultSet 객체로 반환하는 메소드
			...
			//DDL 명령의 처리 결과를 정수값(int)로 반환하는 메소드
