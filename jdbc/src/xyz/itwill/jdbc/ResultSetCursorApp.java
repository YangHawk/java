...
public class ResultSetCursorApp {
	public static void main(String[] args) throws SQLException {
		...
		// Statement.executeQueary(String sql): SELECT 명령을 전달하여 실행하고 실행결과
		// (검색행)을 ResultSet 객체로 반환하는 메소드
		// ResultSet 객체에 저장된 모든 행을 행단위로 처리하기 위애 내부적으로
		...
		// 커서에 대한 이동 방법 및 커서가 위치한 처리행의 조작 설정 가능
		// resultSetType: ResultSet 커서의 이동 관련 값(ResultSet 인터페이스의 상수 사용)을 전달하여 설정
		// → ResultSet.TYPE_FORWARD_ONLY: ResultSet 커서는 다음행으로만 이동 가능 - 기본값
		...
		// resultSetConcurrency: ResultSet 커서가 위치한 처리행의 조작 관련 속성값(ResultSet 인터페이스의 상수
		// 사용) 전달
		// → ResultSet.CONCUR_READ_ONLY: ResultSet 커서가 위치한 처리행 조작 불가능 - 기본값
		...
		// ResultSet.first(): ResultSet 커서를 첫번째 행으로 이동하는 메소드 - 초기값은 BOF
		rs.first();
		System.out.println(rs.getRow() + "행: 학번 = " + rs.getInt("no") + ", 이름 = " + rs.getString("name"));
		...
		// ResultSet 커서가 위치한 처리행을 조작하기 위해서는 검색대상에 [*] 사용 불가능
		// sql = "select * from student order by no";
		 ...
		 * rs.absolute(2);
		 * 
		 * ResultSet.updateXXX(String columnLabeel, XXX value): ResultSet 객체에서 ResultSet
		 * 커서가 위치한 처리행의 컬럼값을 변경하는 메소드 → XXX는 컬럼값을 변경하기 위해 전달받기 위한 Java 자료형
		 * rs.updateString("name", "임꺾쩡");
		 * 
		 * ResultSet.updateRow(): ResultSet 객체에서 ResultSet 커서가 위치되어 변경된 컬럼값이 저장된 행을 실제
		 * 테이블에 적용하여 변경 처리하는 메소드 rs.updateRow();
		 */

		/*
		 * rs.absolute(3);
		 * 
		 * // ResultSet.moveToInsertRow(): ResultSet 객체에서 ResultSet 커서가 위치한 곳에 // 새로운 행을
		 * 삽입하고 기존행은 다음행으로 차례대로 이동 처리하는 메소드 rs.moveToInsertRow();
		 * 
		 * // 새롭게 삽입된 행의 컬럼값을 변경 rs.updateInt("no", 4000); rs.updateString("name",
		 * "전우치"); rs.updateString("phone", "010-1818-1818"); rs.updateString("address",
