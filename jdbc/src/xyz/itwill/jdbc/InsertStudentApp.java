...
public class InsertStudentApp {
	public static void main(String[] args) {
		// JDBC 관련 객체를 저장하기 위한 참조 변수는 try 영역 외부에서 선언
		// → try 영역을 포함한 모든 영역에서 참조 변수를 이용하여 객체를 사용
		Connection con = null;
		Statement stmt = null;
		try {
			...
			Class.forName("oracle.jdbc.driver.OracleDriver"); // 여러 번 호출해도 한번만 등록

			// 2.DriverManager 클래스에 등록된 JDBC Driver 객체를 이용하여 DBMS 서버에 접속
			// 하여 Connection 객체에 저장
			...
			// 4. Statement 객체를 사용하여 SQL 명령(INSERT, UPDATE, DELETE, SELECT)을 DBMS 서버에 전달하여
			// 실행하고 실행결과를
			// 반환받아 저장
			// Statement.executeUpdate(String sql): DML 명령을 전달하여 실행하는 메소드
			// → DML 명령의 실행 결과로 조작행의 갯수를 정수값으로 반환
			// Statement.executeQuery(String sql): SELECT 명령을 전달하여 실행하는 메소드
			// → SELECT 명령의 실행 결과로 검색행이 ResultSet 객체를 반환
			...
			// 예외 발생과 상관 없이 무조건 실행되는 명령을 작성하는 영역
			// 그런데 예외가 발생할 때 stmt, con = null이다. (초기값) / null 가지고 메소드 호출하면
			// NullPointException 발생! 그래서 if문 추가
			try {
				...
				//NullPointerException: 참조변수에 Null이 저장된 상태에서 메소드를 호출할 경우 발생하는 예외
				if (stmt != null)
					stmt.close(); // 변수 선언을 try 바깥에 선언해줘야 함
