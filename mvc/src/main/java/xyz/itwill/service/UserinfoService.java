...
// ▶ 다수의 DAO 클래스의 메소드를 호출하여 데이터 처리 관련 필요한 기능을 제공 - 모듈화
// ▶ 데이터 처리 관련 기능 구현 시 발생되는 모든 문제에 대한 인위적 예외 발생 - Model 클래스에서 예외 처리
public class UserinfoService {
  ...
  // 회원 정보를 전달받아 USERINFO 테이블에 회원 정보를 삽입하는 메소드
  // ▶ 매개 변수로 전달받은 회원 정보의 아이디가 USERINFO 테이블에 저장된 기존 회원 정보의 아이디와 중복될 경우 인위적 예외 발생
  public void addUserinfo(UserinfoDTO userinfo) throws SQLException, ExistsUserinfoException {
    if (UserinfoModelTwoDAO.getDAO().selectUserinfo(userinfo.getUserid()) != null) {
      throw new ExistsUserinfoException("이미 사용중인 아이디를 입력하였습니다."); // 사용자 정의 예외 클래스를 이용하여 인위적 예외 발생
    }
    UserinfoModelTwoDAO.getDAO().insertUserinfo(userinfo);
  }

  // 회원 정보를 전달받아 USERINFO 테이블에 저장된 회원 정보를 변경하는 메소드
  // ▶ 전달받은 회원 정보가 USERINFO 테이블에 없을 경우 인위적 예외 발생
  public void modifyUserinfo(UserinfoDTO userinfo) throws SQLException, UserinfoNotFoundException {
    if (UserinfoModelTwoDAO.getDAO().selectUserinfo(userinfo.getUserid()) == null) {
      throw new UserinfoNotFoundException("회원 정보가 존재하지 않습니다.");
    }
    UserinfoModelTwoDAO.getDAO().updateUserinfo(userinfo);
  }

  // 아이디를 전달받아 USERINFO 테이블에 저장된 회원 정보를 삭제하는 메소드
  // ▶ 매개 변수로 전달받은 아이디의 회원 정보가 USERINFO 테이블에 없는 경우 인위적 예외 발생
  ...
  // 아이디를 전달받아 USERINFO 테이블에 저장된 회원 정보를 검색하여 DTO 객체로 반환하는 메소드
  // ▶ 매개 변수로 전달받은 아이디의 회원 정보가 USERINFO 테이블에 없는 경우 인위적 예외 발생
  ...
  // USERINFO 테이블에 저장된 모든 회원 정보를 검색하여 List 객체로 반환하는 메소드
  public List<UserinfoDTO> getUserinfoList() throws SQLException {
    return UserinfoModelTwoDAO.getDAO().selectUserinfoList();
  }

  // 로그인 정보(아이디와 비밀번호)를 전달받아 인증 처리하는 메소드
  // ▶ 인증 실패 시 인위적 예외 발생 - 예외가 발생하지 않은 경우 인증 성공
