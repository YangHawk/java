...
public class MessagePrintApp {
	public static void main(String[] args) {
		/*
		HelloMessageObject object = new HelloMessageObject(); // 제어의 역행이 아님
		MessagePrint print = new MessagePrint();
		print.setObject(object); // 객체 필드에 객체를 전달하여 저장 - 포함 관계 완성
		print.messagePrint();
		 */
		
		// ApplicationContext 객체: Spring Container 기능을 제공하기 위한 객체
		// ▶ Spring Bean Configuration File(XML 파일) 을 제공받아 객체(Spring Bean) 생성
		ApplicationContext context = new ClassPathXmlApplicationContext("03_message.xml");
		
		//SPring 컨테이너에게 필요한 Spring Bean(객체)를 제공받아 저장
		// ▶ 매개 변수에 Spring Bean을 구분하기 위한 식별자(beanName | beanId)를 전달
