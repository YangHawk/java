...
public class CreateBeanApp {
	public static void main(String[] args) {
		...
		// BeanFactory 인터페이스를 상속받은 자식 클래스로 객체를 생성 - BeanFactory 객체
		// ▶ BeanFactory 객체를 생성할 때 Spring Bean Configuration File을 제공받아 스프링 컨테이너 생성 - 스프링 컨테이너 초기화 작업
		// ▶ 매개 변수에는 Spring Bean Configuration File의 경로를 제공받아 Resource 객체로 생성하여 전달
		// ▶ BeanFactory 객체는 Spring Bean Configuration File에 등록된 클래스로 미리 객체를 생성하지 않고 Spring Bean을 제공받기 위하여 요청 시 객체를 생성하여 제공(※실행해보면 아무런 동작 X)
		BeanFactory factory = new XmlBeanFactory(new FileSystemResource("src/main/resources/04-1_beanCreate.xml"));
		System.out.println("=============== Spring Container 초기화 후 ===============");
		// BeanFactory.getBean(String beanName): 매개 변수에 Spring Bean을 구분하기 위한 식별자(beanName)을 전달하여 스프링 컨테이너가 Spring Bean(객체)를 생성하여 반환하는 메소드
		...
		// ▶ 스프링 컨테이너가 소멸되기 전에 스프링 컨테이너에 의하여 관리되는 모든 객체(Spring Bean)를 자동 소멸
