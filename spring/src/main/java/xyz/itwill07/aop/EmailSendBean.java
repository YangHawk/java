...
// JavaMail 기능을 구현하기 위해 spring-context-support 라이브러리와 javax.mail 라이브러리를 프로젝트에 빌드 처리 - 메이븐 이용: pom.xml

// 메일 전송 기능을 제공하기 위한 클래스 - 메일 서버의 SMTP 서비스를 이용하여 메일 전송
// ▶ 메일 서버(Mail Server): 메일을 송수신하는 서비스를 제공하는 컴퓨터
// ▶ SMTP(Simple Message Transfer Protocol) 서비스로 메일을 보내고 POP3(Post Office Protocol) 서비스나
// IMAP(Internet Message Access Protocol) 서비스로 메일을 받아 사용자에 전달
public class EmailSendBean {
	...
	// 메일을 전송하는 메소드
	// ▶ 메일을 받는 사람의 이메일 주소, 제목, 내용을 매개 변수로 전달받아 저장
	...
	public String sendEmail(String email, String subject, String content) throws Exception {
		...
		// MimeMessage.setText(String content): MimeMessage 객체의 메일 내용(일반 문서 / 텍스트 메세지)을
		// 변경하는 메소드
		// message.setText(content);

		// MimeMessage.setContent(Object o, String type): MimeMessage 객체의 메일 내용을 변경하는
		// 메소드
		// ▶ type 매개 변수에 메일로 전달할 문서의 형식(MimeType)을 전달하여 저장
		...
		// 객체의 메일을 받는 사람의 이메일 주소 관련 정보를 변경하는 메소드
