...
// 입력값 검증(InputData Validation): 사용자 입력값에 대한 유효성 검사를 구현하기 위한 기능
// 1. validation-api / hibernate-validator 라이브러리를  프로젝트에 빌드 처리 - 메이븐: pom.xml
...
public class ValidController {
	...
	// @Valid: Spring 태그에 의하여 전달된 값을 Command 객체의 필드에 저장하기 전에 전달값에 대한 유효청 검증 기능을 제공하기 위한 어노테이션
	// ▶ VO(DTO) 클래스의 필드에 유효성 검증 관련 어노테이션 사용
	// Errors: 유효성 검증 후 발생되는 모든 에러 관련 정보를 저장하기 위한 객체
