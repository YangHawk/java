...
// 3. MultipartHttpServletRequest 객체를 사용하여 [multipart/form-data] 형태로 전달된 값 또는 파일을 처리
...
public class FileController {
	...
	// 요청 처리 메소드의 MultipartHttpServletRequest 인터페이스로 매개 변수를 선언하면 Front Controller에게 MultipartHttpServletRequest 객체를 제공받아 사용 가능
	...
	public String uploadOne(MultipartHttpServletRequest request) throws IOException {
		...
		// MultipartHttpServletRequest.getFile(String name): 전달 파일을 MultipartFile 객체로 반환하는 메소드
		// ▶ MultipartFile 객체: 사용자로부터 입력되어 전달된 파일 정보를 저장하기 위한 객체
		MultipartFile uploadFile = request.getFile("uploadFile");

		// 전달받은 파일에 대한 검증 작성 - JavaScript 갖고 할 수 없음!
		...
		// MultipartFile.getContentType() : MultipartFile 객체에 저장된 파일의 형식(MimeType)을 반환하는 메소드
		System.out.println("파일 형식 = " + uploadFile.getContentType());
		System.out.println("파일 크기 = " + uploadFile.getBytes().length);

		// 전달 파일을 저장하기 위한 서버 디렉토리의 시스템 경로를 반환받아 저장
		...
		// 전달 파일을 서버 디렉토리에 저장하기 위한 파일 정보가 저장된 File 객체 생성
		// File 객체: 시스템(서버)에 존재하는 파일 정보를 저장하기 위한 객체
		// MultipartFile.getOriginalFilename(): MultipartFile 객체에 저장된 전달 파일의 이름을 반환하는 메소드
		String uploadFileName = uploadFile.getOriginalFilename();
		File file = new File(uploadDirectory, uploadFile.getOriginalFilename());

		// MultipartFile.transferTo(File file): MultipartFile 객체에 저장된 전달 파일을 File 객체의 시스템 파일(업로드 파일)로 전달하여 저장하는 메소드
		uploadFile.transferTo(file); // 전달 파일을 서버 디렉토리에 저장 - 업로드 처리
	...
	// 요청 처리 메소드의 매개 변수를 사용하여 전달값 및 전달 파일을 제공받아 사용 가능
	// ▶ 전달값 및 전달 파일의 이름과 같은 이름으로 매개 변수 작성
	// 문제점: 전달 파일의 이름이 서버 디렉토리에 저장된 파일의 이름과 같은 경우 전달 파일로 덮어씌워짐
	// 해결법: 전달 파일의 이름을 서버 디렉토리에 없는 파일 이름으로 변경하여 서버 디렉토리에 저장
	@RequestMapping(value = "/upload1", method = RequestMethod.POST)
	public String uploadOne(@RequestParam String uploaderName, @RequestParam MultipartFile uploadFile, Model model) throws IOException {
		// 전달받은 파일에 대한 검증 작성 - JavaScript 갖고 할 수 없음!
		...
		// 전달 파일을 서버 디렉토리에 저장하기 위한 파일 정보가 저장된 File 객체 생성
		// ▶ 서버 디렉토리에 저장된 파일 이름을 중복되지 않는 이름으로 사용되도록 변경
		// UUID.rendomUUID(): 36byte의 문자열로 구현된 식별자를 생성하여 반환하는 메소드
	...
	public String uploadTwo(@RequestParam String uploaderName, @RequestParam List<MultipartFile> uploadFileList, Model model) throws IOException {
		...
		for (MultipartFile multipartFile : uploadFileList) {
			...
			String uploadFileName = UUID.randomUUID()+"_"+multipartFile.getOriginalFilename();
			File file = new File(uploadDirectory, uploadFileName);
			
			// 전달 파일을 서버 디렉토리에 저장 - 업로드 처리
