...
public class ModelController {
	...
	// @ModelAttribute: 메소드에 @ModelAttribute 어노테이션을 사용하면 메소드에 의하여 반환되는 객체(값)를 현재
	// Controller 클래스에 작성된 모든 요청 처리 메소드의 뷰에게 제공하기 위한 어노테이션
	// ▶ 요청 처리 메소드에 의하여 응답 처리될 뷰에서 공통적으로 사용되는 객체(값)를 제공할 목적으로 사용
	// value 속성: 메소드에 의하여 반환된 객체(값)를 뷰에서 사용하기 위한 이름(속성명)을 속성값으로 설정
