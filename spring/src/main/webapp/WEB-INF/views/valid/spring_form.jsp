...
<%-- Spring 프레임워크에서 제공하는 폼 태그 라이브러리 포함 --%>
...
<body>
	...
	<%-- modelAttribute 속성(필수): 요청 처리 메소드 매개 변수에 저장된 Command 객체의 속성명을 속성값으로 설정 --%>
	<%-- 유효성 검증 실패 시? --%>
	<form:form action = "${url }" method="post" id="joinForm" modelAttribute="employee">
		<table>
			<tr>
				<td>아이디</td>
				<td>
					<%-- Spring input 태그: 문자열을 입력받기 위한 입력 태그 --%>
					<%-- ▶ input 태그의 type="text" 와 동일한 기능 제공 --%>
					<%-- path 속성: 값을 전달하기 위한 이름을 속성값으로 설정 --%>
					<%-- ▶ input 태그의 name="" / id="" 을 동시에 선언하는 것과 동일 --%>
					<%-- ▶ 유효성 검증이 실패한 경우 value=="" 속성을 입려값으로 설정하는 기능 제공 --%>
					<form:input path="id"/>
					<%-- Spring errors 태그: 에러 메세지를 제공받아 출력하는 태그 --%>
					<%-- path 속성: 에러 메세지를 제공받아 출력하기 위한 식별자(전달값의 이름)를 속성값으로 설정 --%>
					<%-- cssClass 속성: 에러 메세지를 출력하기 위한 CSS 스타일의 클래스 선택자를 속성값으로 설정 --%>
					<%-- element 속성: 에러 메세지를 출력하기 위한 태그를 속성값으로 설정 --%>
					<%-- delimiter 속성: 에러 메세지가 여러 개인 경우 구분하기 위한 구분자를 속성값으로 설정 - 기본 구분자 : <br> --%>
