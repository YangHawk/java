...
// 2. 테스트 프로그램에서 사용될 로그 구현체에 대한 환경 설정 파일 변경
// ▶ [src/test/resources] 폴더의 [log4j.xml] 파일의 내용 변경
// 3. [src/test/java] 폴더에 테스트 프로그램 관련 클래스 작성
// ▶ junit 라이브러리와 spring-test 라이브러리의 scope 속성을 주석 처리한 후 테스트 프로그램 관련 클래스 작성 - 테스트 프로그램 관련 클래스 작성 후 주석 제거
...
// @ContextConfiguration: 테스트 프로그램에서 사용될 Spring Bean을 제공하기 위한 Spring Bean Configuration File을 설정하기 위한 어노테이션
...
@ContextConfiguration(locations = {"file:src/main/webapp/WEB-INF/spring/root-context.xml"}) // 이놈이 저 놈을 가져다 써!
public class DataSourceTest {
	// 테스트 클래스의 메소드에서 사용할 객체를 저장하기 위한 피륻 선언
	// ▶ 필드에 @Autowired 어노테이션을 사용하여 의존성 주입 - 생성자를 이용한 의존성 주입 불가
	@Autowired
	private DataSource dataSource;
	
	// @Test: 테스트 메소드를 설정하는 어노테이션 - 테스트 명령을 작성하여 실행
	// ▶ SpringJUnit4ClassRunner 클래스에 의하여 테스트 프로그램 관련 클래스가 객체로 생성된 후 자동 호출되는 메소드
	@Test
